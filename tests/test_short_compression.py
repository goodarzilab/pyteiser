...
def pack_indices_with_struct(inp_array, width):
    N_indices = inp_array.shape[0]
    binary_array = np.zeros((N_indices, 32), dtype=np.bool)

    for i, value in enumerate(inp_array):
    ...
    print(binary_array.sum(axis=1))
    print(binary_array.sum(axis=0))
    #print(uint8_array)
        # np.
        # print(bit_string)
        # print(np_uint)
